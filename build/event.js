...
export function init(ctx) {
    ...
    var _a = ctx.opts, dom = _a.dom, direction = _a.direction, click_el_index = _a.click_el_index, expand_touch_dom = _a.expand_touch_dom;
    var parent = dom.parentElement;
    var grandpa = parent.parentElement;
    var init_event_fn = bind(mousedown, ctx);
    var now_percentage = get_now_percentage(ctx, parent);
    var revise_width = get_width(dom) / 2;
    var revise_height = get_height(dom) / 2;
    var total_x = get_width(grandpa);
    var total_y = get_height(grandpa);
    ...
    ctx.opts = tslib_1.__assign({}, ctx.opts, { parent: parent,
        total_x: total_x,
        total_y: total_y,
...
export function mousedown(e) {
    ...
    var ctx = this;
    var _a = get_client_xy(e), x = _a.x, y = _a.y;
    var _b = ctx.opts, dom = _b.dom, parent = _b.parent, click_instance = _b.click_instance;
    var touchmove = bind(mousemove, ctx);
...
export function mousemove(e) {
    ...
    var _b = get_client_xy(e), x = _b.x, y = _b.y;
    ...
    var precent = get_percent(ctx, left, top);
...
export function dispatch(ctx, event_type, precent, is_animate) {
...
export function set_click_position(ctx, parent) {
    ...
    if (sibling) {
        sibling.onclick = function (event) {
            ...
            var self_random_num = random_number = random_num();
